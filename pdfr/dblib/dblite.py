...
class Adapter():
    ...
    def newEntry(self,form):
        self.db.execute('insert into documents (title,user,source,published,link,comment,timestamp) values (?, ?, ?, ?, ?, ?, ?)',
                 [form['title'], form['user'], form['source'], form['published'], form['link'], form['comment'], time.strftime("%a, %d %b %Y %H:%M:%S %Z")]
                 )
        self.commit()
    def getEntry(self,entry_no):
        curse = self.db.execute('select title,user,link,source,published,comment,timestamp from documents where id=?',(str(entry_no), ))
        row = curse.fetchone()
        return dict(title=row[0], user=row[1], link=row[2],source=row[3],published=row[4], comment=row[5], id=entry_no, posted=row[6])
